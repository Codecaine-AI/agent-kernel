---
covers: "How agents run outside their owning apps: the harness taxonomy (app harnesses vs the terminal harness), the TUI leg's boot contract and catalog precedence, host and tool-policy classification, the core kernel, TUI session tracing, and how any kernel joins the Observatory."
concepts: [harness, tui, terminal-harness, core-kernel, catalog-precedence, host, disallowed-tools, session-binding, trace-ingest, observatory, kernel-checklist]
...
is bun-first. The extension's import graph and every `host:"any"` sidecar
must be Node-clean (no `bun:sqlite`, no `Bun.*`, no `import.meta.dir`), and
sidecars should import deep, dependency-light subpaths rather than package
barrels (a barrel pulling an incompatible typebox flavor at module load has
broken boots). Doctor's host-portability check proves loadability under *a*
Node runtime; the final gate for tools sidecars is a boot through real pi.
...
bundles' home, `packages/core-harness` (`bun run core-harness`, :4860) is
...
2. **Ingest** — `bunx agent-kernel-tui-ingest` (batch, idempotent) scans pi's
...
5. Register: `bun run registry:check|sync` in the observatory repo (Core's
