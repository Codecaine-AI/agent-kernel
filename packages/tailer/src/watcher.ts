...
export class DirectoryWatcher {
  ...
  getSnapshotStats(): { fromSnapshot: number; newFiles: number } {
    ...
    for (const filePath of this.readers.keys()) {
      if (this.cursorStore.hasFile(filePath)) {
        fromSnapshot++;
      } else {
        newFiles++;
      }
  ...
  private createReader(filePath: string): void {
    ...
    const reader = new FileReader(filePath, initialOffset, (fp, events) => {
      this.cursorStore.set(fp, reader.getOffset());
      this.onEvents(fp, events);
    });
