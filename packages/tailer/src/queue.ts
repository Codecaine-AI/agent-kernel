  ...
  private async flushBatch(batch: TraceEvent[]): Promise<void> {
    ...
    for (let attempt = 0; attempt < this.config.maxRetries; attempt++) {
      try {
        ...
        console.error(
          `Flush failed (attempt ${attempt + 1}/${this.config.maxRetries}, size=${batch.length}): ${msg}. Retrying in ${delay}ms`,
        );
    ...
    if (batch.length === 1) {
      ...
      console.error(
        `[queue] quarantining bad event ${bad.eventId} (type=${bad.type}, session=${bad.appSessionId}): ${msg}`,
      );
      return;
    }
